...
graphFileType = "pdf" 
myDataFrame = read.csv( file="Salary.csv" )  #,stringsAsFactors = TRUE
...
myDataFrame= myDataFrame[myDataFrame$Org %in% c('PSY','CHEM','ENG','BFIN'),]
myDataFrame$Org= as.factor(myDataFrame$Org)
myDataFrame$LogSalary= log10(myDataFrame$Salary)
str(myDataFrame)
...
x1contrasts = list( 
  list( c("Full") , c("Assoc") , compVal=0.0 , ROPE=NULL ) ,
  list( c("Assoc") , c("Assis") , compVal=0.0 , ROPE=NULL ) 
)
x2contrasts = list( 
  list( c("CHEM") , c("ENG") , compVal=0.0 , ROPE=NULL ) ,
  list( c("CHEM") , c("PSY") , compVal=0.0 , ROPE=NULL ) ,
  list( c("BFIN") , c("PSY","CHEM","ENG") , compVal=0.0 , ROPE=NULL ) 
)
...
source("Jags-Ymet-Xnom2fac-MnormalHom_mod2.R")
#------------------------------------------------------------------------------- 
