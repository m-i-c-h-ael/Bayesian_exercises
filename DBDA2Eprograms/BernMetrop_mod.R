...
myData = c(0,1,1) #c()  # c(rep(0,6),rep(1,14))
...
trajectory[1] = .99 #0.01 # arbitrary value
...
traj= acceptedTraj
acf(traj,lag.max=30,col='skyblue',lwd=3 )
Len= length(traj)
lag=10
trajHead= traj[(1+lag):Len]
trajTail= traj[1:(Len-lag)]
#plot(1:(Len-lag),trajTail)
#points((1:lag):Len)
plot(trajHead,trajTail, main= paste('proposalSD=',proposalSD,'cor=',
              round(cor(trajHead,trajTail),3)))
   #, main= bquote( list( "Prpsl.SD"== .(proposalSD)),
                          #            lag== .(lag),
